  ...
  const cards = [
    ...
    },
    {
    ...
    },
     {
      ...
      value: events?.reduce((acc, curr) => acc + (curr.capacity - curr.availableTickets), 0) || 0,
        ...
        {cards.map((card) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className={`p-3 rounded-xl bg-gradient-to-br ${card.gradient} text-white shadow-lg shadow-${card.color}-500/20`}>
                {card.icon}
              </div>
              <span className={`text-xs font-semibold px-2 py-1 rounded-full bg-${card.color}-50 dark:bg-${card.color}-900/20 text-${card.color}-600 dark:text-${card.color}-400`}>
                +2.5%
              </span>
            </div>
            <div>
