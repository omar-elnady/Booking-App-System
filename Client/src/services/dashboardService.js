...
export const dashboardService = {
  ...
  getOrganizerRequests: async () => {
    const response = await apiClient.get("/user/organizer-requests");
    return response.data;
  },

  handleOrganizerRequest: async ({ userId, status }) => {
    const response = await apiClient.patch("/user/handle-organizer-request", {
      userId,
      status,
    });
    return response.data;
  ...
  getCategoryRequests: async () => {
    const response = await apiClient.get("/categories/pending");
    return response.data;
  },

  handleCategoryRequest: async ({ categoryId, status }) => {
    const response = await apiClient.patch("/categories/handle-request", {
      categoryId,
      status,
    });
    return response.data;
