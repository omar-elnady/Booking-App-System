    ...
    <>
      ...
      >
        <div className="rounded-2xl mx-auto flex md:items-stretch flex-col md:flex-row justify-between bg-white dark:bg-black border border-gray-400 dark:border-gray-600 md:h-[300px] relative select-none">
          <div
            className={`md:w-[45%] w-full md:h-full h-[250px] flex-shrink-0 relative`}
          >
            {/* Category Badge - On Image */}
            <span
              className={`absolute z-20 ${
                lang === "ar" ? "right-4 top-4" : "left-4 top-4"
              } dark:bg-white bg-black text-white dark:text-black px-2 py-1 rounded text-sm`}
            >
              {categoryName}
            </span>

            {/* Wishlist Heart Button - On Image (Opposite Side) */}
            <button
              onClick={handleWishlist}
              className={cn(
                "absolute z-20 p-2 rounded-full backdrop-blur-sm transition-all hover:scale-110",
                lang === "ar" ? "left-4 top-4" : "right-4 top-4",
                isInWishlist
